...
use crate::{AppContext, BidAskContract, SourceFeedSerializer, SourceFeedSerializerNew, BidAskContractNew};
...
#[async_trait::async_trait]
impl SocketEventCallback<BidAskContractNew, SourceFeedSerializerNew> for CallbackNew {
    async fn handle(
        &self,
        connection_event: ConnectionEvent<BidAskContractNew, SourceFeedSerializerNew>,
    ) {
        match connection_event {
            ConnectionEvent::Connected(connection) => {
                let mut write_access = self.app.new_connections.lock().await;
                println!("New connection {}", connection.id);
                write_access.insert(connection.id, connection);
            }
            ConnectionEvent::Disconnected(connection) => {
            ...
            ConnectionEvent::Payload {
                ...
                if payload.is_ping() {
                    connection.send(BidAskContractNew::Pong).await;
                }
                println!("Received payload from {:?}", payload);
            }
        }
